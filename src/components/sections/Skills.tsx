        ...
        <div className="grid md:grid-cols-2 gap-8">
          {skills.map((group, groupIndex) => (
            <motion.div
              ...
              className="p-6 bg-card-bg border border-card-border rounded-xl hover:border-accent/20 transition-colors duration-300"
            >
              ...
              <div className="flex flex-wrap gap-2">
                {group.items.map((skill) => (
                  <span
                    key={skill}
                    className="px-3 py-1.5 text-xs font-mono bg-background border border-card-border rounded-md text-foreground/80 hover:border-accent/40 hover:text-accent transition-all duration-200"
                  >
                    {skill}
                  </span>
                ))}
